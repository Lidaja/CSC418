...
// Render settings
enum { WIREFRAME, SOLID, OUTLINED, MATTE, METALIC };	// README: the different render styles
...
void pressKeyTwo(int key, int x, int y);
...
void pressKey(unsigned char key, int x, int y){
//if(key=='w') {ball_z = ball_z + 1;}
if(key=='a') {if (ball_x > -10){
		    ball_x = ball_x - 1;}    
	      }
//if(key=='s') {ball_z = ball_z - 1;}
if(key=='d') {if (ball_x < 10){
		    ball_x = ball_x + 1;}    
	      }
}
...
void pressKeyTwo(int key, int x, int y){
if(key=='w') {ball_z = ball_z + 1;}
if(key=='a') {ball_x = ball_x - 1;}
if(key=='s') {ball_z = ball_z - 1;}
if(key=='d') {ball_x = ball_x + 1;}
}
...
void initGlui()
{
	...
	glui_light_pos = GLUI_Master.create_glui("Light", 0, 0, Win[1]+612);
	
	glui_panel = glui_joints->add_panel("Light source position", GLUI_PANEL_NONE);
	glui_spinner = glui_keyframe->add_spinner_to_panel(glui_panel, "light position:", GLUI_SPINNER_FLOAT, &light_theta);
...
void animate()
{
	...
	if( frameRateTimer->elapsed() > SEC_PER_FRAME )
	{
		...
		ball_y = ball_y + .2 * y_velocity;
		
		ball_z = ball_z + direction;
...
void display(void)
{
	...
	if(renderStyle == MATTE){
	 GLfloat light0Position[] = {0, 20, 0 ,20};
	}
	...
	glTranslatef(joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_X),
				...
				joint_ui_data->getDOF(Keyframe::ROOT_TRANSLATE_Z) - 3);
				
	glRotatef(joint_ui_data->getDOF(Keyframe::ROOT_ROTATE_X) + 15,1,0,0);
...
{
	glBegin(GL_QUADS);
		// draw front faceSOLID
		glNormal3d(0, 0, 1);
		glVertex3f(-1 * x_factor,  0.0, 0.0);
		glVertex3f( x_factor,  0.0, 0.0);
		glVertex3f( x_factor,  2.0, 0.0);
		glVertex3f(-1 * x_factor,  2.0, 0.0);
...
{	
	...
	//glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_ROLL),0,1,0);
	//glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_YAW),0,0,1);
	glTranslatef(0.0, -0.0, 0.0);
  
	glBegin(GL_QUADS);
		// draw front face
		glNormal3d(0, 0, 1);
		glVertex3f(-0.2 + ball_x,  1.8  + ball_y, 0.2);
		glVertex3f( 0.2 + ball_x,  1.8  + ball_y, 0.2);
		glVertex3f( 0.2 + ball_x,  2.2  + ball_y, 0.2);
		glVertex3f(-0.2 + ball_x,  2.2  + ball_y, 0.2);
...
void drawRightLeg()
	...
	glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_YAW),0,0,1);
	glTranslatef(-0.0, -0.5, 0.0);
...
void drawLeftLeg()
	...
	glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_YAW),0,0,1); 
	glTranslatef(-0.0, -0.5, 0.0);
	glBegin(GL_QUADS);
		// draw front face
		glNormal3d(0, 0, 1);
...
void drawBeakBottom()
{
	...
	glBegin(GL_QUADS);
		...
		// draw right face	  glColor3f(1.0, 1.0, 1.0);
		glNormal3d(1, 0, 0);
		glVertex3f( 0.5, -0.2,  0.2);
		glVertex3f( 0.5, -0.2, -0.2);
		glVertex3f( 0.5,  0.2, -0.2);
...
void drawPenguin(){		
	...
	GLfloat whiter[4] = {0.4f, 0.4f, 0.4f, 1.0f};
	GLfloat blackish[4] = {0.1f, 0.1f, 0.1f, 1.0f};
	GLfloat orange[4] = {1.0f, 01.0f, 0.1f, 1.0f};
	GLfloat cyan[] = {0.f, .8f, .8f, 1.f};
	GLfloat metal[] =  {100};
	GLfloat white[4] = {0.8f, 0.8f, 0.8f, 1.0f};
	GLfloat black[4] = {0.1f, 0.1f, 0.1f, 1.0f};
	GLfloat matte[] = {10};
	//Draw Court
	glPushMatrix();
		  
	  
		glRotatef(00.0, 0.0, 0.0, 0.0);
		glRotatef(00.0, 00.0, 00.0, 0.0);
		...
		//lighting
		if(renderStyle == MATTE){
		  glMaterialfv ( GL_FRONT, GL_SHININESS, matte);
		  glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, black);
		}
		else if(renderStyle == METALIC){
		  glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, white);
		  glMaterialfv ( GL_FRONT, GL_SHININESS, metal);
		}
		glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, blackish);
		glMaterialfv(GL_FRONT, GL_DIFFUSE, blackish);
