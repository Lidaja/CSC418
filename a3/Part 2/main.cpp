...
//Our function to run every frame
void update(int data)
{
  glutTimerFunc(3, update, -1);

  //Setting the joint animations to make up the swing animation
  if (Swing1_p1){
...
void display(void)
{
	...
	//Update ball's position each frame
	ball_z += velocity_z;
	ball_y += velocity_y;
	ball_x += velocity_x;
	...
	//Check if either player has hit ball
...
//Our keyboard handler to perform certain events depending on what key has been pressed
void pressKey(unsigned char key, int x, int y){
...
//Drawing everything but the player
void drawWorld()
{
...
//Cube to use for creating the world
void drawCube()
{
	glBegin(GL_QUADS);
		// draw front face
