...
const float LIGHT_HEIGHT_MAX     = 50.0;
const float LIGHT_HEIGHT_MIN     = -50.0;
const float LIGHT_RAD_MIN	  = 1;
const float LIGHT_RAD_MAX	  = 5;
const float LIGHT_THETA_MAX	  = 2*PI;
const float LIGHT_THETA_MIN	  = 0;
...
void initGlui()
{
	...
	glui_spinner->set_speed(SPINNER_SPEED*3);
	...
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "Light Height", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_HEIGHT));
	glui_spinner->set_float_limits(LIGHT_HEIGHT_MIN, LIGHT_HEIGHT_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "Light Radius", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_RAD));
	glui_spinner->set_float_limits(LIGHT_RAD_MIN, LIGHT_RAD_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED);
	
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "Light Theta", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_THETA));
	glui_spinner->set_float_limits(LIGHT_THETA_MIN, LIGHT_THETA_MAX, GLUI_LIMIT_CLAMP);
	glui_spinner->set_speed(SPINNER_SPEED2);	
...
void display(void)
{
	...
	// SAMPLE CODE **********
	//
	//int render = get_int_val(glui_radio_group);
	...
	else if (renderStyle == MATTE)
	{
		...
		GLfloat light_pos[] = {joint_ui_data->getDOF(Keyframe::LIGHT_RAD)*cos(joint_ui_data->getDOF(Keyframe::LIGHT_THETA)),joint_ui_data->getDOF(Keyframe::LIGHT_HEIGHT),joint_ui_data->getDOF(Keyframe::LIGHT_RAD)*sin(joint_ui_data->getDOF(Keyframe::LIGHT_THETA)),1};
...
void drawPenguin(int c)
{
  glPushMatrix();
		...
		glPushMatrix();
		    ...
		    glScalef(1/0.4*0.25,-1/0.7*0.25,1);
		...
		glPushMatrix();
			...
			glPushMatrix();
			    ...
			    glTranslatef(0,-3.5+joint_ui_data->getDOF(Keyframe::BEAK)*5,0);
			    drawCube();
			glPopMatrix();
		glPopMatrix();
